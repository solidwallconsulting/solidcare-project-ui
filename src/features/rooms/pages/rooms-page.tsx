import { useState } from "react";
import { useNavigate } from "@tanstack/react-router";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { CalendarRange, DoorOpen, MoreHorizontal, Pencil, Plus, Trash2, Users } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { PageContainer } from "@/shared/components/layout/page-container";
import { SearchInput } from "@/shared/components/forms/search-input";
import { FilterSelect } from "@/shared/components/forms/filter-select";
import { ConfirmDialog } from "@/shared/components/feedback/confirm-dialog";
import { StatusBadge, type StatusTone } from "@/shared/components/feedback/status-badge";
import { EmptyState, ErrorState, LoadingState } from "@/shared/components/feedback/states";
import { DataTablePagination } from "@/shared/components/data-table/data-table";
import { useLookups } from "@/shared/hooks/use-lookups";
import { cn } from "@/lib/utils";
import {
  roomsApi,
  roomKeys,
  RoomFormDialog,
  roomStatusLabels,
  roomTypeLabels,
  roomTypes,
  type Room,
  type RoomFormValues,
  type RoomListQuery,
} from "@/features/rooms";
import { RoomPlanningSheet } from "@/features/rooms/components/room-planning-sheet";

const DEFAULT_PAGE_SIZE = 10;

const statusTone: Record<Room["status"], StatusTone> = {
  available: "success",
  occupied: "warning",
  reserved: "info",
  in_surgery: "warning",
  sterilizing: "info",
  maintenance: "danger",
};

export function RoomsPage() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { departmentName, departmentOptions } = useLookups();

  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<"all" | Room["status"]>("all");
  const [departmentId, setDepartmentId] = useState("all");
  const [roomType, setRoomType] = useState<"all" | Room["roomType"]>("all");
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(DEFAULT_PAGE_SIZE);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [planningRoom, setPlanningRoom] = useState<Room | undefined>();
  const [editing, setEditing] = useState<Room | undefined>();
  const [deleting, setDeleting] = useState<Room | undefined>();

  const query: RoomListQuery = {
    search,
    status,
    departmentId,
    roomType,
    page,
    pageSize,
    sortBy: "name",
    sortDir: "asc",
  };

  const list = useQuery({
    queryKey: roomKeys.list(query),
    queryFn: () => roomsApi.list(query),
  });

  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: roomKeys.all });
  };

  const saveMutation = useMutation({
    mutationFn: async (values: RoomFormValues) => {
      if (editing) return roomsApi.update(editing.id, values);
      return roomsApi.create({
        ...values,
        createdAt: new Date().toISOString().slice(0, 10),
      });
    },
    onSuccess: () => {
      toast.success(editing ? "Salle mise à jour" : "Salle créée");
      setDialogOpen(false);
      setEditing(undefined);
      invalidate();
    },
    onError: () => toast.error("Impossible d'enregistrer la salle"),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => roomsApi.remove(id),
    onSuccess: () => {
      toast.success("Salle supprimée");
      setDeleting(undefined);
      invalidate();
    },
    onError: () => toast.error("Suppression impossible"),
  });

  const items = list.data?.items ?? [];

  return (
    <PageContainer className="space-y-4">
      <div className="sticky top-14 z-20 -mx-4 border-b border-border/80 bg-background/95 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/85 sm:-mx-6 sm:px-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <SearchInput
            value={search}
            onChange={(value) => {
              setSearch(value);
              setPage(1);
            }}
            placeholder="Rechercher une salle…"
            label="Recherche salles"
            className="min-w-0 flex-1 sm:max-w-none"
          />
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <FilterSelect
              label="Statut"
              value={status ?? "all"}
              onChange={(value) => {
                setStatus(value as "all" | Room["status"]);
                setPage(1);
              }}
              options={Object.entries(roomStatusLabels).map(([value, label]) => ({
                value,
                label,
              }))}
              allLabel="Tous les statuts"
              className="w-full min-w-[9rem] sm:w-40"
            />
            <FilterSelect
              label="Type"
              value={roomType}
              onChange={(value) => {
                setRoomType(value as "all" | Room["roomType"]);
                setPage(1);
              }}
              options={roomTypes.map((type) => ({
                value: type,
                label: roomTypeLabels[type],
              }))}
              allLabel="Tous les types"
              className="w-full min-w-[9rem] sm:w-40"
            />
            <FilterSelect
              label="Département"
              value={departmentId}
              onChange={(value) => {
                setDepartmentId(value);
                setPage(1);
              }}
              options={departmentOptions.map((dep) => ({ value: dep.id, label: dep.label }))}
              allLabel="Tous les départements"
              className="w-full min-w-[10rem] sm:w-48"
            />
            <Button
              className="w-full sm:w-auto"
              onClick={() => {
                setEditing(undefined);
                setDialogOpen(true);
              }}
            >
              <Plus className="size-4" />
              Ajouter
            </Button>
          </div>
        </div>
      </div>

      {list.isLoading ? <LoadingState /> : null}
      {list.isError ? (
        <ErrorState
          description="Impossible de charger les salles."
          onRetry={() => void list.refetch()}
        />
      ) : null}
      {!list.isLoading && !list.isError && items.length === 0 ? (
        <EmptyState title="Aucune salle" description="Ajoutez une salle de consultation." />
      ) : null}

      {!list.isLoading && !list.isError && items.length > 0 ? (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {items.map((room) => (
            <article
              key={room.id}
              role="button"
              tabIndex={0}
              onClick={() => void navigate({ to: "/rooms/$id", params: { id: room.id } })}
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ") {
                  event.preventDefault();
                  void navigate({ to: "/rooms/$id", params: { id: room.id } });
                }
              }}
              className={cn(
                "group relative flex cursor-pointer flex-col gap-3 rounded-xl border border-border bg-card p-4 text-left outline-none transition-colors",
                "hover:border-primary/25 hover:bg-accent/40 focus-visible:ring-2 focus-visible:ring-ring",
              )}
            >
              <div className="flex items-start gap-3">
                <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/8 text-primary">
                  <DoorOpen className="size-5" aria-hidden="true" />
                </span>
                <div className="min-w-0 flex-1 pr-6">
                  <p className="truncate font-display text-[15px] font-semibold tracking-[-0.01em]">
                    {room.name}
                  </p>
                  <p className="truncate text-xs text-muted-foreground">{room.code}</p>
                </div>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="absolute top-2.5 right-2 size-8 opacity-70 group-hover:opacity-100"
                      onClick={(event) => event.stopPropagation()}
                      aria-label={`Actions ${room.name}`}
                    >
                      <MoreHorizontal className="size-4" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" onClick={(event) => event.stopPropagation()}>
                    <DropdownMenuItem
                      onClick={() => {
                        setPlanningRoom(room);
                      }}
                    >
                      <CalendarRange className="size-4" />
                      Planning
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => {
                        setEditing(room);
                        setDialogOpen(true);
                      }}
                    >
                      <Pencil className="size-4" />
                      Modifier
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      className="text-destructive focus:text-destructive"
                      onClick={() => setDeleting(room)}
                    >
                      <Trash2 className="size-4" />
                      Supprimer
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>

              <div className="flex flex-wrap items-center gap-2">
                <StatusBadge label={roomStatusLabels[room.status]} tone={statusTone[room.status]} />
                <span className="text-xs text-muted-foreground">{roomTypeLabels[room.roomType]}</span>
              </div>

              <div className="mt-auto space-y-2 border-t border-border/70 pt-3 text-sm">
                <p className="truncate text-muted-foreground">
                  {departmentName(room.departmentId)}
                </p>
                <p className="flex items-center gap-1.5 text-muted-foreground">
                  <Users className="size-3.5 shrink-0" aria-hidden="true" />
                  <span>
                    Étage {room.floor} · cap. {room.capacity}
                  </span>
                </p>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  className="w-full"
                  onClick={(event) => {
                    event.stopPropagation();
                    setPlanningRoom(room);
                  }}
                >
                  <CalendarRange className="size-4" />
                  Voir le planning
                </Button>
              </div>
            </article>
          ))}
        </div>
      ) : null}

      <DataTablePagination
        page={list.data?.page ?? page}
        totalPages={list.data?.totalPages ?? 1}
        total={list.data?.total ?? 0}
        pageSize={list.data?.pageSize ?? pageSize}
        onPageChange={setPage}
        onPageSizeChange={(size) => {
          setPageSize(size);
          setPage(1);
        }}
        pageSizeOptions={[10, 15, 20, 25]}
      />

      <RoomFormDialog
        open={dialogOpen}
        onOpenChange={setDialogOpen}
        room={editing}
        isSaving={saveMutation.isPending}
        onSubmit={async (values) => {
          await saveMutation.mutateAsync(values);
        }}
      />

      <RoomPlanningSheet
        open={Boolean(planningRoom)}
        room={planningRoom}
        onOpenChange={(open) => {
          if (!open) setPlanningRoom(undefined);
        }}
      />

      <ConfirmDialog
        open={Boolean(deleting)}
        onOpenChange={(open) => {
          if (!open) setDeleting(undefined);
        }}
        title="Supprimer la salle ?"
        description={`« ${deleting?.name ?? ""} » sera retirée de l'inventaire.`}
        confirmLabel="Supprimer"
        cancelLabel="Annuler"
        destructive
        onConfirm={() => {
          if (deleting) deleteMutation.mutate(deleting.id);
        }}
      />
    </PageContainer>
  );
}
