...
import { planningEventsApi, shiftsApi } from "@/features/planning/api";
import {
...
const statusTone: Record<Doctor["status"], StatusTone> = {
  available: "success",
  on_leave: "warning",
  inactive: "danger",
};
...
export function DoctorPlanningSheet({
  open,
  onOpenChange,
  doctor,
}: {
  ...
  const slots = useMemo(() => {
    ...
    const roomName = (id?: string) =>
      id ? (roomsQuery.data?.find((room) => room.id === id)?.name ?? id) : "";
    const events = (eventsQuery.data ?? [])
      .filter((event) => event.doctorId === doctor.id)
      .map((event) => eventToSlot(event, roomName(event.roomId ?? event.operatingRoomId)));
    const shifts = (shiftsQuery.data ?? [])
      .filter(
        (shift) =>
          shift.doctorId === doctor.id ||
          (shift.resourceType === "doctor" && shift.resourceId === doctor.id),
      )
      .map(shiftToSlot);
    const appointments = (appointmentsQuery.data ?? [])
      .filter((item) => item.doctorId === doctor.id)
      .map((item) => appointmentToSlot(item, patientName(item.patientId)));
    return [...events, ...shifts, ...appointments].sort((a, b) =>
      `${a.date}${a.startTime}`.localeCompare(`${b.date}${b.startTime}`),
    );
  }, [appointmentsQuery.data, doctor, eventsQuery.data, patientName, roomsQuery.data, shiftsQuery.data]);
  ...
  const isLoading = eventsQuery.isLoading || shiftsQuery.isLoading || appointmentsQuery.isLoading;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
          ...
          <div className="flex flex-wrap items-center gap-2 pr-8">
            <div className="min-w-0">
              <SheetTitle className="font-display text-base leading-none tracking-[-0.02em]">
                {doctor ? `Dr. ${doctor.firstName} ${doctor.lastName}` : "Planning"}
              </SheetTitle>
              <SheetDescription className="mt-1 text-foreground">
                {doctor ? specialtyLabels[doctor.specialty] : "Aucun médecin sélectionné."}
              </SheetDescription>
            </div>
            {doctor ? (
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          {!doctor ? (
            <div className="p-5">
              <EmptyState title="Aucun médecin sélectionné." />
            </div>
          ) : isLoading ? (
            <div className="p-5">
              <LoadingState />
  ...
  return (
    <>
      ...
      >
        ...
        <span className="mt-0.5 block truncate text-[11px] font-medium text-foreground">{slot.meta}</span>
      </button>
    </>
  );
}

function SlotDetail({ slot }: { slot: Slot | null }) {
  if (!slot) {
    return (
      <div className="flex h-full min-h-48 flex-col justify-center">
        <EmptyState
          title="Sélectionnez un créneau"
          description="Le détail s'affiche ici : horaire, type, statut et notes."
        />
      </div>
    );
  }
  ...
  const statusLabel =
    slot.kind === "shift"
      ? shiftStatusLabels[slot.status === "in_progress" ? "scheduled" : slot.status]
      : slot.kind === "event"
        ? planningEventStatusLabels[slot.status]
    ...
    <div className="space-y-4">
      <div>
        <p className="text-[11px] font-semibold tracking-wide uppercase">
          {slot.kind === "appointment" ? "Rendez-vous" : slot.kind === "shift" ? "Vacation" : "Créneau"}
        </p>
      ...
      <dl className="space-y-2.5 text-sm">
        ...
        <div>
          <dt className="text-xs">Contexte</dt>
          <dd className="font-medium">{slot.meta}</dd>
        </div>
        <div>
...
function eventToSlot(event: PlanningEvent, room: string): Slot {
  return {
    id: event.id,
    title: event.title,
    date: event.date,
    ...
    kind: "event",
    notes: event.notes,
    meta: room || "Salle non renseignée",
  };
}
...
function shiftToSlot(shift: ShiftAssignment): Slot {
  return {
    ...
    status: shift.status === "completed" ? "completed" : shift.status === "cancelled" ? "cancelled" : "scheduled",
    kind: "shift",
    meta: shiftTypeLabels[shift.shift],
  };
}

function appointmentToSlot(item: Appointment, patient: string): Slot {
  ...
  return {
    ...
    startTime: start,
    endTime: addMinutes(start, item.durationMinutes),
    status:
    ...
    kind: "appointment",
    notes: item.reason,
    meta: item.room ? `Salle ${item.room}` : "Rendez-vous",
  };
}
