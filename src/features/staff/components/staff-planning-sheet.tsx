...
type Slot = {
  ...
  startTime: string;
  endTime: string;
  status: Exclude<ShiftStatus, never>;
...
export function StaffPlanningSheet({
  open,
  onOpenChange,
  member,
}: {
  ...
  const shiftsQuery = useQuery({
    queryKey: ["planning", "staff-shifts", member?.id],
    ...
    return (shiftsQuery.data ?? [])
      .filter(
        (shift) =>
          ...
          (shift.resourceType === "nurse" && shift.resourceId === member.id),
      )
      .flatMap(shiftToSlots)
      .sort((a, b) => `${a.date}${a.startTime}`.localeCompare(`${b.date}${b.startTime}`));
  }, [member, shiftsQuery.data]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
          ...
          <div className="flex flex-wrap items-center gap-2 pr-8">
            <div className="min-w-0">
              <SheetTitle className="font-display text-base leading-none tracking-[-0.02em]">
                {member ? fullName(member.firstName, member.lastName) : "Planning"}
              </SheetTitle>
              <SheetDescription className="mt-1 text-foreground">
                {member
                  ? `${staffRoleLabels[member.role]} · ${departmentName(member.departmentId)}`
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          {!member ? (
            <div className="p-5">
              <EmptyState title="Aucun membre sélectionné." />
            </div>
          ...
          return (
            ...
            >
              ...
              <p className="text-[10px] font-medium text-foreground">
                {count === 0 ? "Libre" : `${count} vacation${count > 1 ? "s" : ""}`}
              </p>
            </div>
          );
        })}
...
function SlotDetail({ slot }: { slot: Slot | null }) {
  if (!slot) {
    return (
      <div className="flex h-full min-h-48 flex-col justify-center">
        <EmptyState
          title="Sélectionnez une vacation"
          description="Le détail s'affiche ici : horaire, type et statut."
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-[11px] font-semibold tracking-wide uppercase">Vacation</p>
      ...
      <dl className="space-y-2.5 text-sm">
        ...
        <div>
          ...
          <dd className="mt-1">
            <StatusBadge label={shiftStatusLabels[slot.status]} tone={eventTone(slot.status)} />
          </dd>
        </div>
        <div>
          <dt className="text-xs">Type</dt>
...
function shiftToSlots(shift: ShiftAssignment): Slot[] {
  const overnight = minutesOf(shift.endTime) <= minutesOf(shift.startTime);
  const base: Slot = {
    id: shift.id,
    title: shift.title,
    date: shift.date,
    startTime: shift.startTime,
    endTime: overnight ? "23:00" : shift.endTime,
    status: shift.status,
    shift: shift.shift,
    ...(overnight ? { notes: `Garde de nuit jusqu'au lendemain ${shift.endTime}` } : {}),
  };
  if (!overnight) return [base];
  return [
    base,
    {
      id: `${shift.id}-next`,
      title: `${shift.title} (fin)`,
      date: addDays(shift.date, 1),
      startTime: "06:00",
      endTime: shift.endTime,
      status: shift.status,
      shift: shift.shift,
      notes: `Suite de la garde commencée le ${shift.date}`,
    },
  ];
}
